...
export function filterPosts (posts, isTimeline) {
  posts = posts.filter((item, index) => {
    ...
    if (posts.indexOf(item) !== index) {
      ...
      if(useInstance().$site.locales != undefined){
        const locales = Object.keys(useInstance().$site.locales)
        locales.shift()
        const localeCondition = useInstance().$localePath === '/' ? locales.every((locale) => {
          return path.startsWith(locale)
        }) : !path.startsWith(useInstance().$localePath)
        
        someConditions = someConditions || localeCondition
      }
