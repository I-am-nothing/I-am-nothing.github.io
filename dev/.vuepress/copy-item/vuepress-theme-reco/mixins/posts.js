    ...
    $categoriesList () {
      return this.$categories.list.map(category => {
        category.pages = category.pages.filter(page => {
          return page.frontmatter.publish !== false && localeCondition(page)
        })
        if(category.pages.length != 0){
          if(category.path.split('/')[1] != category.pages[0]['regularPath'].split('/')[1]) {
            category.path = category.path.replace('/' + category.path.split('/')[1],'')
          }
        }
        return category
      })
    ...
    $tagesList () {
      return this.$tags.list.map(tag => {
        ...
        if(tag.pages.length != 0){
          if(tag.path.split('/')[1] != tag.pages[0]['regularPath'].split('/')[1]) {
            tag.path = tag.path.replace('/' + tag.path.split('/')[1],'')
          }
        }
        return tag
      })
...
function localeCondition(page) {
  const locales = Object.keys(useInstance().$site.locales)
  locales.shift()
  return useInstance().$localePath === '/' ? locales.every((locale) => {
    return !page.path.startsWith(locale)
  }) : page.path.startsWith(useInstance().$localePath)
}
